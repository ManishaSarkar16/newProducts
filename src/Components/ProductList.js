      ...
      <div className='product-row'>
        {items.map(item => (
          <div key={item.id} className='product-col'>
            <div className='heading-content'><div className='product-title'>{item.name}</div> <div> ₹{item.price}</div></div>
            ...
            <button className='add-to-cart-button' onClick={() => addToCart(item.id)}>Add to Cart</button>
          </div>
        ))}
      </div>
    </div>
